...
`backend/utils/` contains various utility modules. In practice, you'll probably:
...
To check if this is being used correctly, if you search your codebase, _all_ uses of `process.env` in `backend/` should be in this file.
...
- ESLint's `"no-console"` rule is activated in `backend/`, to remind you to use the logger instead

## `middleware.js`

Defines default middleware used in `backend/app.js`:

- `clientRouter`: brings the React client into the Express server for production mode
  - serves React app build outputs (CSS, HTML, JS, etc.) from `backend/static/` directory
